// netlify/functions/extract-teklif-background.js
//
// Arka plan fonksiyonu: Netlify, "-background" son ekli fonksiyonları 202 ile hemen
// yanıtlar ve çalışmayı arkada sürdürür (Pro planda 15 dakikaya kadar). Bu, Kasko gibi
// uzun/çok sayfalı PDF'lerde normal 26 saniyelik senkron sınırı aşmamak için gerekli.
// Sonuç, Netlify Blobs üzerinde jobId ile saklanır; tarayıcı tarafı extract-status.js
// fonksiyonunu birkaç saniyede bir yoklayarak (polling) sonucu alır.
...
exports.handler = async (event) => {
  // @netlify/blobs modülünü fonksiyon gövdesi içinde, try/catch korumasıyla yüklüyoruz.
  // Modül yüklenemezse (örn. bağımlılık kurulum sorunu) en azından anlaşılır bir hata
  // dönebiliyoruz — önceki sürümde bu, tanısı zor bir HTTP 500'e yol açıyordu.
  let getStore;
  try {
    ({ getStore } = require('@netlify/blobs'));
  } catch (e) {
    console.error('KRİTİK: @netlify/blobs modülü yüklenemedi:', e.message);
    return; // Background function: bu noktada client'a bilgi ulaştıramayız, sadece loglarız.
  }

  let pdfBase64, fileName, jobId, store;
  try {
    store = getStore('teklif-jobs');
  } catch (e) {
    console.error('KRİTİK: getStore çağrısı başarısız:', e.message);
    return;
  }
  ...
  try {
    ...
    console.error('İstek gövdesi ayrıştırılamadı:', e.message);
    return;
  }

  if (!jobId) { console.error('jobId eksik'); return; }
  ...
  try {
    ...
    if (!apiKey) {
      await store.setJSON(jobId, { status: 'error', error: 'ANTHROPIC_API_KEY tanımlı değil. Netlify > Environment variables kısmından ekleyin.' });
      return;
    }
    if (!pdfBase64) {
      await store.setJSON(jobId, { status: 'error', error: 'pdfBase64 alanı eksik.' });
      return;
    }
    ...
    if (!anthropicRes.ok) {
      ...
      await store.setJSON(jobId, { status: 'error', error: 'Anthropic API hatası (' + anthropicRes.status + '): ' + errText });
      return;
    }
    ...
    if (!textBlock) {
      await store.setJSON(jobId, { status: 'error', error: 'Yapay zeka yanıtında metin bulunamadı.' });
      return;
    }
    ...
    try {
      ...
      await store.setJSON(jobId, { status: 'error', error: 'Yapay zeka çıktısı JSON olarak ayrıştırılamadı: ' + e.message });
      return;
    }
    ...
    try {
      await store.setJSON(jobId, { status: 'error', error: 'Beklenmeyen hata: ' + err.message });
    } catch (e2) {
      console.error('Hata durumu bile blob\'a yazılamadı:', e2.message);
    }
  }
};
